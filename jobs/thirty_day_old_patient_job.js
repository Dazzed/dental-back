  ...
  try {
    ...
    const thirtyDayOldPatients = await db.User.findAll({
      where: {
        createdAt: {
          $between: [
            oneMonthBeforeToday.set('H', 0).set('m', 0).set('s', 0).format('YYYY-MM-DD HH:mm:ss Z'),
            oneMonthBeforeTodayCopy.set('H', 23).set('m', 59).set('s', 59).format('YYYY-MM-DD HH:mm:ss Z'),
          ]
        },
        type: 'client',
        hash: {
